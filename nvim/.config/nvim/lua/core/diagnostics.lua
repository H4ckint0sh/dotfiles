vim.diagnostic.config({
	...
	float = { header = "", border = "rounded", source = true },
	virtual_text = false,
...
local has_tags = function(diagnostic)
	return diagnostic._tags and (diagnostic._tags.deprecated or diagnostic._tags.unnecessary) or false
end
	...
	return {
		show = function(namespace, bufnr, diagnostics, opts)
			diagnostics = vim.tbl_filter(function(diagnostic)
				...
				return diagnostic.severity == vim.diagnostic.severity.ERROR or has_tags(diagnostic)
			end, diagnostics)
...
vim.diagnostic.handlers.underline = underline_handler(vim.diagnostic.handlers.underline)
